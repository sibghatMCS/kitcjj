...
for($i=1;$i<= $month; $i++){
   ...
   $month_name = date('F', strtotime("-$mon month"));
    ...
    //echo $month_name;
    echo'<a href="index.php?r=attandance/Generatepayslip&month='.$i.'&id='.$_GET['id'].'">
...
?>
    ...
    <br/>
     ...
     <h3 class="header smaller lighter blue">PAY SHEET OF <?php echo $sql_emp['name']; ?> FOR THE MONTH OF <?php echo  date("F", mktime(null, null, null, $mon));; ?></h3>
   ...
   <table width="100%" cellspacing="2" cellpadding="2" style="border : 1px solid;"> 
       <tr  style="border : 1px solid;">
           ...
           <th  style="border : 1px solid; width: 300px">Remarks</th>
     ...
     if(count($data)>0)
     {
       ...
       <tr  style="border : 1px solid;">
           ...
           <td  style="border : 1px solid;text-align: center;width: 100px;"><?php echo $rec['service']; ?></td>
           <td  style="border : 1px solid;text-align: center;"><?php if($rec['remarks'] == '') echo '-'; else echo $rec['remarks']; ?></td>
           <td  style=" border : 1px solid;text-align: center;text-align: center;"><?php if($rec['attd_status'] == 1) {
               echo 'Present';
           }
           else
               echo 'Absent' ; ?></td>
       ...
       <tr  style="border : 1px solid;background-color: gainsboro;">
          <td  style="border : 1px solid;text-align: center;" ><?php echo $rec['date']; ?></td>
           <td style="border : 1px solid;text-align: center;"><?php if($rec['overtime_amount'] == 0) echo '-'; else echo $rec['overtime_amount']; ?></td>
           <td  style="border : 1px solid;text-align: center;"><?php if($rec['fine_amount'] == 0) echo '-'; else echo $rec['fine_amount']; ?></td>
           <td  style="border : 1px solid;text-align: center;width: 100px;">
               <?php 
           echo $rec['service']  
                   ?>
           </td>
           <td  style="border : 1px solid;text-align: center;"><?php if($rec['remarks'] == '') echo '-'; else echo $rec['remarks']; ?></td>
           <td colspan="5" style="border : 1px solid;text-align: center; font-weight: bold"><?php echo 'ABSENT'; ?></td>
       </tr>
           <?php     }   
       ...
       <?php }else {  ?>
       <tr  style="border : 1px solid;background-color: gainsboro;">
           <td colspan="6" style="text-align: center">No Record </td></tr> <?php } ?>
   ...
   <table width="50%" cellspacing="2" cellpadding="2" style="border : 1px solid;"> 
       ...
       <tr  style="border : 1px solid;">
          ...
          <tr  style="border : 1px solid;">
            <td  style="border : 1px solid;">Total Service Amount</td>
            <td  style="border : 1px solid;"><?php  echo $sum['sum(service)']?></td>
...
<td  style="border : 1px solid;font-weight: bold"><?php  $tol_sal=$sum['sum(service)']+$sql_emp['salary']+$sum['sum(overtime_amount)'] - $toatl_dedcut ; echo $tol_sal  ?></td>
...
?>
          ...
          <form action="" method="post">
            ...
            <input type="hidden" name="salary" value="<?php echo $tol_sal ?> " />
            <table class="table" style="width:50%">
            <tr><td>Balance Amount</td><td><?php echo $balc.'+'.$tol_sal.'='; echo $balc+$tol_sal   ?></td></tr>
      <?php       if(count($loan)>0)
{ ?>
            
            <input type="hidden" name="loan_id" value="<?php echo $loan->id ?>" />
  <tr><td>Loan Amount :</td><td><?php echo $loan->amount ?></td></tr>
  <tr><td>Monthly Installment :</td><td><input type="text" name="installment" value="<?php echo $loan->installment ?>" /></td></tr>
  <tr><td colspan="2"><table style="width:100%"><tr><td>Load Sanction Date</td><td><?php echo $loan->date ?></td></tr>
              <tr><td>Amount Paid </td> 
                  <td><?php  $sql_l="select sum(amount) as amt from loan_receive where loan_id =$loan->id "; 
              
                 $command = Yii::app()->db->createCommand($sql_l);
$sql_l= $command->queryRow();
...
<?php } ?>
              ...
              <tr><td>Amount Paying now:</td><td><input type="text" name="amount" /></td></tr>
